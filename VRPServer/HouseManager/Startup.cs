...
namespace HouseManager
{
    internal class Startup
    {
        ...
        {
            app.Map("/websocketcomand", Comand);
        ...
        private static void Comand(IApplicationBuilder app)
        {
            app.Run(async context =>
            {
                if (context.Request.Method.ToString().ToLower() == "post" || context.Request.Method.ToString().ToLower() == "get")
                {
                    try 
                    {
                        /*
                         * 首先要有房间ID，playerID，sessionID
                         * 第一个业务，是获取当前房间中游戏这的数量。
                         * 第二个业务，是设置停止时间。
                         * 第三个业务，是登录房间
                         * 第四个业务，是匹配任务。
                         * 第五个业务，是消息转发。
                         ...
                         * 
                         */
                        var fromIPAndPort = context.Request.Form["fromIPAndPort"].ToString();
                    ...
                    //var jsonStr = File.ReadAllText($"{BasePath}worldposition.json");
                    //result = Newtonsoft.Json.JsonConvert.DeserializeObject<List<PositionItem>>(jsonStr);
                    //var objsPath = Directory.GetFiles(BasePath, "*.obj");
                    //for (var i = 0; i < objsPath.Length; i++)
                    //{
                    //    var fileName = Path.GetFileName(objsPath[i]);
                    //    Console.WriteLine($"{fileName}");

                    //    if (result.Count(item => item.id == fileName.Split('.')[0]) == 0)
                    //    {
                    ...
                    //    }
                    //}
                    //result = result.OrderBy(item => item.id).OrderByDescending(item => item.show).ToList();
                    ////   result=result.OrderByDescending(item=> new FileInfo(item+".obj").Length).to
                    //jsonStr = Newtonsoft.Json.JsonConvert.SerializeObject(result);
                    //File.WriteAllText($"{BasePath}worldposition.json", jsonStr);
                    //await context.Response.WriteAsync(Newtonsoft.Json.JsonConvert.SerializeObject(result));
                }
            });
        }
