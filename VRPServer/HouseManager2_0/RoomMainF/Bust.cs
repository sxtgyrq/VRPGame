...
#warning 这里要在web前台进行提示
        ...
        private async void setBustF(int startT, commandWithTime.bustSet bustSet)
        {
            lock (this.PlayerLock)
            {
            ...
            lock (this.PlayerLock)
            {
                    ...
                    && car.state == CarState.roadForAttack)
                {
                    ...
                    else
                    {
                        ...
                        var attackMoney = 0;
                        Console.WriteLine($"player:{player.Key},car,bustMoney:{0}");
                                ...
                                // if (m1 != m2)
                                {
                                    ...
                                    if (victim.TheLargestHolderKey == player.Key)
                                    {
                                        ...
                                        }
                                        {
                                            ...
                                            {
                                                player.PromoteDiamondCount[rewardType] += rewardCount;
                                                SendPromoteCountOfPlayer(rewardType, player, ref notifyMsg);
